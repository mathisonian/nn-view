...
function LeNet(selector) {
  ...
  function redraw({architecture, architecture2}={}) {
  ...
  function redistribute({betweenLayers_=betweenLayers,
      ...
      let layer_widths = architecture.map((layer, i) => (layer['numberOfSquares']-1) * betweenSquares + layer['squareWidth']);
      layer_widths = layer_widths.concat(lenet.fc_layers.map((layer, i) => layer['size']));

      let largest_layer_width = Math.max(...layer_widths);
