...
function LeNet(selector) {
  ...
  var nWise = function nWise(n, array) {
    iterators = Array(n).fill().map(function () {
      return array[Symbol.iterator]();
    });
    iterators.forEach(function (it, index) {
      return Array(index).fill().forEach(function () {
        return it.next();
      });
    });
  ...
  var pairWise = function pairWise(array) {
  ...
  var betweenLayers = [];
  var betweenLayersDefault = 12; // numberOfSquares, squareWidth, stride.
  ...
  var textFn = function textFn(layer) {
    return _typeof(layer) === "object" ? layer['numberOfSquares'] + '@' + layer['squareWidth'] + 'x' + layer['squareWidth'] : "1x" + layer;
  };
  ...
  function redraw() {
    var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
        ...
        architecture2 = _ref.architecture2;

    lenet.rects = architecture.map(function (layer, layer_index) {
      return range(layer['numberOfSquares']).map(function (rect_index) {
        return {
          'id': layer_index + '_' + rect_index,
          'layer': layer_index,
          'rect_index': rect_index,
          'side': layer['squareWidth']
        };
      });
    });
    ...
    lenet.convs = architecture.map(function (layer, layer_index) {
      return Object.assign({
        'id': 'conv_' + layer_index,
        'layer': layer_index
      }, layer);
    });
    lenet.convs.pop();
    lenet.convs = lenet.convs.map(function (conv) {
      return Object.assign({
        'x_rel': rand(0.1, 0.9),
        'y_rel': rand(0.1, 0.9)
      }, conv);
    });
    lenet.conv_links = lenet.convs.map(function (conv) {
      return [Object.assign({
    ...
    rect = rect.enter().append("rect").attr("class", "rect").attr("id", function (d) {
      return d.id;
    ...
    conv = conv.enter().append("rect").attr("class", "conv").attr("id", function (d) {
      return d.id;
    }).attr("width", function (d) {
      return d.stride;
    }).attr("height", function (d) {
      return d.stride;
    }).style("fill-opacity", 0).merge(conv);
    link = link.data([]);
    link.exit().remove();
    ...
    poly = poly.enter().append("polygon").attr("class", "poly").attr("id", function (d) {
      return d.id;
    }).merge(poly);
    line = line.data([]);
    line.exit().remove();
    line = line.data(lenet.fc_links);
    line = line.enter().append("line").attr("class", "line").attr("id", function (d) {
      return d.id;
    }).merge(line);
    text = text.data([]);
    text.exit().remove(); // text = text.data(label);
    // text = text.enter()
    //            .append("text")
  ...
  function redistribute() {
    var _ref2 = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
        _ref2$betweenLayers_ = _ref2.betweenLayers_,
        betweenLayers_ = _ref2$betweenLayers_ === void 0 ? betweenLayers : _ref2$betweenLayers_,
        _ref2$betweenSquares_ = _ref2.betweenSquares_,
        betweenSquares_ = _ref2$betweenSquares_ === void 0 ? betweenSquares : _ref2$betweenSquares_;

    betweenLayers = betweenLayers_;
    betweenSquares = betweenSquares_;
    var layer_widths = architecture.map(function (layer, i) {
      return (layer['numberOfSquares'] - 1) * betweenSquares + layer['squareWidth'];
    });
    layer_widths = layer_widths.concat(lenet.fc_layers.map(function (layer, i) {
      return layer['size'];
    }));
    var largest_layer_width = Math.max.apply(Math, _toConsumableArray(layer_widths));
    var layer_x_offsets = layer_widths.reduce(function (offsets, layer_width, i) {
      return offsets.concat([offsets.last() + layer_width + (betweenLayers[i] || betweenLayersDefault)]);
    }, [0]);
    var layer_y_offsets = layer_widths.map(function (layer_width) {
    ...
    var x = function x(layer, node_index) {
      return layer_x_offsets[layer] + node_index * betweenSquares + screen_center_x;
    };

    var y = function y(layer, node_index) {
      return layer_y_offsets[layer] + node_index * betweenSquares + screen_center_y;
    };

    rect.attr('x', function (d) {
      return x(d.layer, d.rect_index);
    }).attr('y', function (d) {
      return y(d.layer, d.rect_index);
    });

    var xc = function xc(d) {
      return layer_x_offsets[d.layer] + (d['numberOfSquares'] - 1) * betweenSquares + d['x_rel'] * (d['squareWidth'] - d['stride']) + screen_center_x;
    };

    var yc = function yc(d) {
      return layer_y_offsets[d.layer] + (d['numberOfSquares'] - 1) * betweenSquares + d['y_rel'] * (d['squareWidth'] - d['stride']) + screen_center_y;
    };

    conv.attr('x', function (d) {
      return xc(d);
    }).attr('y', function (d) {
      return yc(d);
    });
    link.attr("x1", function (d) {
      return xc(d) + d['stride'];
    }).attr("y1", function (d) {
      return yc(d) + (d.i ? 0 : d['stride']);
    }).attr("x2", function (d) {
      return layer_x_offsets[d.layer + 1] + (architecture[d.layer + 1]['numberOfSquares'] - 1) * betweenSquares + architecture[d.layer + 1]['squareWidth'] * d.x_rel + screen_center_x;
    }).attr("y2", function (d) {
      return layer_y_offsets[d.layer + 1] + (architecture[d.layer + 1]['numberOfSquares'] - 1) * betweenSquares + architecture[d.layer + 1]['squareWidth'] * d.y_rel + screen_center_y;
    });
    poly.attr("points", function (d) {
      return layer_x_offsets[d.layer] + screen_center_x + ',' + (layer_y_offsets[d.layer] + screen_center_y) + ' ' + (layer_x_offsets[d.layer] + screen_center_x + 10) + ',' + (layer_y_offsets[d.layer] + screen_center_y) + ' ' + (layer_x_offsets[d.layer] + screen_center_x + d.size + 10) + ',' + (layer_y_offsets[d.layer] + screen_center_y + d.size) + ' ' + (layer_x_offsets[d.layer] + screen_center_x + d.size) + ',' + (layer_y_offsets[d.layer] + screen_center_y + d.size);
    });
    line.attr("x1", function (d) {
      return layer_x_offsets[d.layer - 1] + (d.i ? 0 : layer_widths[d.layer - 1]) + d.prevSize + screen_center_x;
    }).attr("y1", function (d) {
      return layer_y_offsets[d.layer - 1] + (d.i ? 0 : layer_widths[d.layer - 1]) + screen_center_y;
    }).attr("x2", function (d) {
      return layer_x_offsets[d.layer] + (d.i ? 0 : d.size) + screen_center_x;
    }).attr("y2", function (d) {
      return layer_y_offsets[d.layer] + (d.i ? 0 : d.size) + screen_center_y;
    });
    text.attr('x', function (d) {
      return (layer_x_offsets[d.layer] + layer_widths[d.layer] + layer_x_offsets[d.layer + 1] + layer_widths[d.layer + 1] / 2) / 2 + screen_center_x - 15;
    }).attr('y', function (d) {
      return layer_y_offsets[0] + screen_center_y + largest_layer_width;
    });
    info.attr('x', function (d) {
      return layer_x_offsets[d.layer] + screen_center_x;
    }).attr('y', function (d) {
      return layer_y_offsets[d.layer] + screen_center_y - 15;
    });
  }

  function style() {
    var _ref3 = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
        _ref3$color1_ = _ref3.color1_,
        color1_ = _ref3$color1_ === void 0 ? color1 : _ref3$color1_,
        _ref3$color2_ = _ref3.color2_,
        color2_ = _ref3$color2_ === void 0 ? color2 : _ref3$color2_,
        _ref3$borderWidth_ = _ref3.borderWidth_,
        borderWidth_ = _ref3$borderWidth_ === void 0 ? borderWidth : _ref3$borderWidth_,
        _ref3$rectOpacity_ = _ref3.rectOpacity_,
        rectOpacity_ = _ref3$rectOpacity_ === void 0 ? rectOpacity : _ref3$rectOpacity_,
        _ref3$showLabels_ = _ref3.showLabels_,
        showLabels_ = _ref3$showLabels_ === void 0 ? showLabels : _ref3$showLabels_;
    ...
    rect.style("fill", function (d) {
      return d.rect_index % 2 ? color1 : color2;
    });
